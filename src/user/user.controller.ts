...
export class UserController {
    ...
    async registerStudent(@Body() createStudentDto: CreateStudentDto, @Request() req: any): Promise<any> {
    ...
    @ApiResponse({ status: 500, description: 'Internal server error' })
    async registerFaculty(@Body() createFacultyDto: CreateFacultyDto, @Request() req: any): Promise<any> {
    ...
    @ApiResponse({ status: 201, description: 'Recruiter  registered successfully' })
    @ApiResponse({ status: 400, description: 'Invalid request' })
    @ApiResponse({ status: 409, description: 'Recruiter  already exists' })
    @ApiResponse({ status: 500, description: 'Internal server error' })
    async registerRequiter(@Body() createRecruiterDto: CreateRecruiterDto, @Request() req: any): Promise<any> {
